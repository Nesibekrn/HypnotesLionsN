...
public class US_030BaranStepDefs extends CommonPage {
    ...
    public void user_goes_to_therapist_register_page() {
        driver.get(LINKS.THERAPIST_REGISTER_URL.getLink());
    }

    @When("user submits a valid name to the Full Name section")
    public void user_submits_a_valid_name_to_the_full_name_section() {
        getRegisterPage().input_fullName.sendKeys(faker.name().fullName());
    }

    @When("user submits a valid email to the Email section")
    public void user_submits_a_valid_email_to_the_email_section() {
        getRegisterPage().input_email.sendKeys(faker.internet().emailAddress());
    }

    @When("user submits a valid passowrd to the Password section")
    public void user_submits_a_valid_passowrd_to_the_password_section() {
    ...
    public void user_verifies_header_is_displayed(String header) {
    ...
    @When("user submits another invalid email to the Email section")
    public void user_submits_another_invalid_email_to_the_email_section() {
        getRegisterPage().input_email.sendKeys("john123@gmail");
    }

    @Then("user verifies the pop-up message is {string}")
    public void user_verifies_the_pop_up_message_is(String message) {
